...
public class addflights extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        // ✅ Back button HTML
        out.println("<a href='flights_admin.html' style='position:absolute; top:20px; right:20px; "
                + "padding:8px 16px; background:#007bff; color:white; text-decoration:none; border-radius:5px; "
                + "font-weight:bold;'>Back to Admin Flights Dashboard</a><br><br>");
        ...
        try {
            ...
            String checkSql = "SELECT COUNT(*) FROM flights WHERE flight_id = ?";
            PreparedStatement checkPs = con.prepareStatement(checkSql);
            checkPs.setString(1, flightId);
            ResultSet rs = checkPs.executeQuery();
            rs.next();
            int count = rs.getInt(1);

            if (count > 0) {
                out.println("<h3 style='color:red'>Flight ID already exists. Please use a different ID.</h3>");
            } else {
                // ✅ INSERT with seats_available
                String sql = "INSERT INTO flights(flight_id, flight_name, source, destination, travel_date, price, seats_available) VALUES(?,?,?,?,?,?,?)";
                ...
                if (i > 0) {
                    out.println("<h3 style='color:green'>Flight Added Successfully!</h3>");
                } else {
                    out.println("<h3 style='color:red'>Failed to Add Flight.</h3>");
                }
            }
