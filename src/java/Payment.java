...
public class Payment extends HttpServlet {
    protected void doPost(HttpServletRequest req, HttpServletResponse res)
        ...
        out.println(".back-btn{width:100%;padding:12px;border:none;border-radius:8px;background:#6c757d;color:#fff;font-size:16px;margin-top:10px;cursor:pointer}");
        ...
        out.println("<div><b>Flight ID:</b> " + esc(flightId) + "</div>");
        out.println("<div><b>Seats:</b> " + numPassengers + "</div>");
        out.println("<div><b>Amount:</b> ₹" + String.format("%,d", total) + "</div>");
        out.println("</div>");

        out.println("<form action='ConfirmBooking' method='post'>");
        // hidden fields to carry forward
        out.println("<input type='hidden' name='flightId' value='" + esc(flightId) + "'/>");
